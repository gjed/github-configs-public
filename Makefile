...
plan-repo:
	...
	cd terraform && terraform plan -target='module.github_org.module.repositories["$(REPO)"]' -out=tfplan
	@echo ""
	@echo "Plan saved to $(PLAN_FILE)"
	@echo "Run 'make plan-show' to view the plan or 'make apply' to apply"
...
apply-repo:
	...
	cd terraform && terraform apply -target='module.github_org.module.repositories["$(REPO)"]'
